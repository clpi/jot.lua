...
function Mod.create(name, imports)
  ...
  if imports then
    for _, import in ipairs(imports) do
      ...
      if not Mod.setup_mod(fullpath) then
      ...
      new_mod.imported[fullpath] = Mod.loaded_mod[fullpath]
    end
  end
...
function Mod.setup_mod_from_table(m)
    ...
    for _, req_mod in ipairs(mod_load.wants) do
      ...
      if not Mod.is_mod_loaded(req_mod) then
        if config.user.mod[req_mod] then
          ...
          if not Mod.setup_mod(req_mod) then
...
function Mod.setup_mod(mod_name, cfg)
  ...
  return Mod.setup_mod_from_table(modl)
end
