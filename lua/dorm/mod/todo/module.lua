...
local module = mod.create("todo")

module.private = {
    namespace = vim.api.nvim_create_namespace("dorm/todo"),
...
--- Attaches the introspector to a given dorm buffer.
...
function module.public.attach_introspector(buffer)
    ...
    module.required["treesitter"].execute_query(
        [[
    (_
    ...
    vim.api.nvim_buf_attach(buffer, false, {
        on_lines = vim.schedule_wrap(function(_, buf, _, first)
            ...
            ---@type TSNode?
            local node = module.required["treesitter"].get_first_node_on_line(buf, first)
            ...
            local node_above = module.required["treesitter"].get_first_node_on_line(buf, first - 1)
