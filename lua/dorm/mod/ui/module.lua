...
module.public = {
    ...
    create_split = function(name, config, height)
        ...
        module.public.apply_buffer_options(buf, vim.tbl_extend("keep", config or {}, base_options))
    ...
    create_vsplit = function(name, enter, buf_config, win_config)
        ...
        -- Merge the user provided options with the base options and apply them to the new buffer
        module.public.apply_buffer_options(buf, vim.tbl_extend("keep", buf_config or {}, base_options))
    ...
    --- Creates a new dorm buffer in a split or in the main window
    ---@param name string the name of the buffer *without* the .dorm extension
    ---@param split_type string "vsplitl"|"vsplitr"|"split"|"nosplit" - if suffixed with "l" vertical split will be spawned on the left, else on the right. "split" is a horizontal split.
    ---@param config table|nil a table of { option = value } pairs that set buffer-local options for the created dorm buffer
    ---@param opts table|nil
    ---   - opts.keys (boolean)             if false, will not use the base keys
    ---   - opts.del_on_autocmd (table)    delete buffer on specified autocmd
    create_dorm_buffer = function(name, split_type, config, opts)
        vim.validate({
            name = { name, "string" },
