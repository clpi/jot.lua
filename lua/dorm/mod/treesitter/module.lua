--[[
    ...
    summary: A module designed to integrate Treesitter into dorm.
...
- `dorm.treesitter.next.heading` - jump to the next heading
- `dorm.treesitter.next.link` - jump to the next link
- `dorm.treesitter.previous.heading` - jump to the previous heading
- `dorm.treesitter.previous.link` - jump to the previous link
--]]

local dorm = require("dorm")
local lib, log, modules, utils = dorm.lib, dorm.log, dorm.mod, dorm.utils

local module = modules.create("treesitter")
...
module.setup = function()
  return { success = true, requires = { "hl" } }
end
...
module.public = {
  ...
  ---@param filetype string? file type of the file or `dorm` if omitted
  ...
  get_document_metadata = function(source, no_trim)
    ...
    local dorm_tree = dorm_parser:parse()[1]
    if not dorm_tree then
      return
    end
    ...
    local meta_node
    for id, node in dorm_query:iter_captures(dorm_tree:root(), iter_src) do
      if dorm_query.captures[id] == "tag_name" then
    ...
    local dorm_meta_parser = vim.treesitter.get_string_parser(meta_source, "dorm_meta")

    local dorm_meta_tree = dorm_meta_parser:parse()[1]

    if not dorm_meta_tree then
      return
    end

    for id, node in meta_query:iter_captures(dorm_meta_tree:root(), meta_source) do
  ...
  --- Parses a query and automatically executes it for dorm
  ...
  execute_query = function(query_string, callback, source, start, finish)
    ...
    local root = dorm_parser:parse()[1]:root()
  ...
  ---Create a dorm TS parser from the given source
  ---@param source string | number | PathlibPath file path or buf number or 0 for current buffer
  ---@return vim.treesitter.LanguageTree? dorm_parser
  ...
  get_ts_parser = function(source)
    ...
    return dorm_parser, iter_src
  end,
}

-- this fixes the problem of installing dorm ts parsers on macOS without resorting to using gcc
local function install_dorm_ts()
  local install = require("nvim-treesitter.install")

  if vim.fn.has("macunix") == 1 then
    -- https://github.com/nvim-dorm/tree-sitter-dorm/issues/7
...
module.on_event = function(event)
  if event.split_type[2] == "sync" then
    ...
    if not ok then
      utils.notify(string.format([[Unable to auto-install dorm parser: %s]], err), vim.log.levels.WARN)
    end

    local install = require("nvim-treesitter.install")
    install.commands.TSInstallSync["run!"]("dorm_meta")
  end
end
