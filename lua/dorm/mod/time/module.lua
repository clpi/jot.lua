--[[
    ...
    summary: Parses and handles dates in dorm.
    internal: true
    ---
`core.time` is an internal module specifically designed
...
- `dorm.time.insert-date` - Insert date at cursor position (called from normal mode)
- `dorm.time.insert-date.insert-mode` - Insert date at cursor position (called from insert mode)
--]]

local d = require("dorm")
local lib, modules, utils = d.lib, d.mod, d.utils

local module = modules.create("time")
...
module.public = {
  ...
  --- Converts a lua `osdate` to a dorm date.
  ...
  ---@param input string #The input which should follow the date specification found in the dorm spec.
  ...
  insert_date = function(insert_mode)
    ...
    if modules.is_module_loaded("calendar") then
      vim.cmd.stopinsert()
      modules.get_module("calendar").select_date({ callback = vim.schedule_wrap(callback) })
    else
      vim.ui.input({
        prompt = "Date: ",
      }, callback)
...
module.load = function()
  vim.keymap.set("", "<Plug>(dorm.time.insert-date)", lib.wrap(module.public.insert_date, false))
  vim.keymap.set("i", "<Plug>(dorm.time.insert-date.insert-mode)", lib.wrap(module.public.insert_date, true))
end
