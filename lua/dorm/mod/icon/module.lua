--[[
    ...
    description: The conceal module converts verbose markup elements into beautified icons for your viewing pleasure.
...
To reduce the amount of cognitive load required to "parse" dorm documents with your own eyes, this module
...
local module = mod.create("conceal", {
  "basic", "diamond"
})
...
module.private = {
  ns_icon = vim.api.nvim_create_namespace("dorm-conceals"),
  ns_prettify_flag = vim.api.nvim_create_namespace("dorm-conceals.prettify-flag"),
...
local function table_get_base_last(tbl, index)
...
local function get_parsed_query_lazy()
  ...
  module.private.prettify_query = utils.ts_parse_query("dorm", query_combined)
...
local function prettify_range(bufid, row_start_0b, row_end_0bex)
  ...
  local treesitter_module = module.required["treesitter"]
...
local function get_table_base_empty(tbl, key)
  if not tbl[key] then
    tbl[key] = {}
  end
  return tbl[key]
...
local function update_cursor(event)
  local cursor_record = get_table_base_empty(module.private.cursor_record, event.buffer)
...
local function handle_init_event(event)
  ...
  local language_tree = vim.treesitter.get_parser(event.buffer, "dorm")
...
local event_handlers = {
  ["cmd.events.base.conceal.toggle"] = handle_toggle_prettifier,
  -- ["autocmd.events.bufnewfile"] = handle_init_event,
...
module.on_event = function(event)
  ...
  if (not module.private.enabled) and (event.type ~= "cmd.events.base.conceal.toggle") then
    return
  end
...
module.load = function()
  local icon_preset =
      module.imported[module.name .. ".preset_" .. module.config.public.icon_preset].config.private
  ...
  -- module.required["autocmd"].enable_autocommand("BufNewFile")
