...
local config, lib, log, mod = dorm.config, dorm.lib, dorm.log, dorm.mod
...
module.private = {
    ...
    open_diary = function(time, custom_date)
        -- TODO(vhyrro): Change this to use dorm dates!
    ...
    create_toc = function()
        ...
        vim.loop.fs_scandir(workspace_path .. config.pathsep .. folder_name .. config.pathsep, function(err, handle)
            ...
            while true do
                -- Name corresponds to either a YYYY-mm-dd.dorm file, or just the year ("nested" strategy)
...
module.config.public = {
    ...
    -- May be "flat" (`2022-03-02.dorm`), "nested" (`2022/03/02.dorm`),
