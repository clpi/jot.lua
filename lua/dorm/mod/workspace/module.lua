...
module.load = function()
    ...
    if module.config.public.open_last_workspace and vim.fn.argc(-1) == 0 then
        if module.config.public.open_last_workspace == "base" then
            if not module.config.public.base_workspace then
                log.warn(
                    'Configuration error in `base.workspace`: the `open_last_workspace` option is set to "base", but no base workspace is provided in the `base_workspace` configuration variable. baseing to opening the last known workspace.'
                )
...
module.private = {
    ...
    current_workspace = { "base", Path.cwd() },
}
...
module.public = {
    ...
    create_file = function(path, workspace, opts)
        ...
        local destination = (fullpath / path):add_suffix(".dorm")
    ...
    get_dorm_files = function(workspace_name)
        ...
        for path in workspace:fs_iterdir(true, 20) do
            if path:is_file(true) and path:suffix() == ".dorm" then
