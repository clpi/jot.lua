--[[
    ...
    summary: A set of utilities for the `base.workspace` module.
    internal: true
    ---
This internal submodule implements some basic utility functions for [`base.workspace`](@base.workspace).
...
local module = dorm.mod.create("workspace.utils")
...
module.public = {
    ...
    expand_pathlib = function(path, raw_path, host_file)
        ...
        if custom_workspace_path then
            ---@type base.workspace
            local workspace = mod.get_module("workspace")
            if not workspace then
                log.error(table.concat({
                    "Unable to jump to link with custom workspace: `base.workspace` is not loaded.",
    ...
    ---@param path string|PathlibPath # path
    ---@param raw_path boolean? # If true, returns resolved path, otherwise, returns resolved path and append ".dorm"
