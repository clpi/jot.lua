...
The TOC module exposes a single command - `:Dorm toc`. This command can be executed with one of three
...
local dorm = require("dorm.core")
local modules, utils, log = dorm.modules, dorm.utils, dorm.log

local module = modules.create("toc")

module.setup = function()
  return {
    requires = { ".treesitter", "core.ui" },
  }
end
...
module.load = function()
  modules.await("core.dormcmd", function(dormcmd)
...
module.public = {
  parse_toc_macro = function(buffer)
    ...
    local success = module.required[".treesitter"].execute_query(
      [[
                (infirm_tag
                    (tag_name) @name
      ...
      function(query, id, node)
        ...
        if
            capture_name == "name"
            and module.required[".treesitter"].get_node_text(node, buffer):lower() == "toc"
        then
          toc = true
        elseif capture_name == "parameters" and toc then
          toc_name = module.required[".treesitter"].get_node_text(node, buffer)
          return true
        end
      end,
      buffer
    ...
    local success = module.required[".treesitter"].execute_query(
      ...
      function(query, id, node)
        ...
        if prefix and title then
          local prefix_text =
              module.required[".treesitter"].get_node_text(prefix, original_buffer)
          local title_text =
              module.required[".treesitter"].get_node_text(title, original_buffer)
  ...
  update_toc = function(toc_title, ui_data, norg_buffer)
    ...
    local norg_root = module.required[".treesitter"].get_document_root(norg_buffer)
...
local function create_ui(tabpage, split_dir, enter)
  ...
  toc_namespace = toc_namespace or vim.api.nvim_create_namespace("dorm/toc")
