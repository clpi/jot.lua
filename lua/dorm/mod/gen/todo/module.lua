...
- `<Plug>(dorm.qol.todo-items.todo.task-done)` (`<LocalLeader>td`)
- `<Plug>(dorm.qol.todo-items.todo.task-undone)` (`<LocalLeader>tu`)
- `<Plug>(dorm.qol.todo-items.todo.task-pending)` (`<LocalLeader>tp`)
- `<Plug>(dorm.qol.todo-items.todo.task-on_hold)` (`<LocalLeader>th`)
- `<Plug>(dorm.qol.todo-items.todo.task-cancelled)` (`<LocalLeader>tc`)
- `<Plug>(dorm.qol.todo-items.todo.task-recurring)` (`<LocalLeader>tr`)
- `<Plug>(dorm.qol.todo-items.todo.task-important)` (`<LocalLeader>ti`)
- `<Plug>(dorm.qol.todo-items.todo.task-cycle)` (`<C-Space>`)
- `<Plug>(dorm.qol.todo-items.todo.task-cycle-reverse)` (no default keybind)
...
local log, modules = dorm.log, dorm.modules

local module = modules.create("todo_items")
...
module.load = function()
  for _, task in ipairs({
    ...
    vim.keymap.set(
      "",
      string.format("<Plug>(dorm.qol.todo-items.todo.task-%s)", task),
...
module.private = {
  ...
  update_parent = function(buf, line, recursion_level)
    ...
    -- TODO(vhyrro):
    -- Implement a toggleable behaviour where dorm can automatically convert this:
    --     * (@ Mon 5th Feb) Test
    --     ** ( ) Test
    -- To this:
    --     * (x|@ Mon 5th Feb) Test
    ...
    local range = module.required["integrations.treesitter"].get_node_range(first_status_extension)
  ...
  get_todo_item_from_cursor = function(buf, line)
    local node_at_cursor = module.required["integrations.treesitter"].get_first_node_on_line(buf, line)
  ...
  make_all = function(buf, node, todo_item_type, char)
    ...
    if not first_status_extension then
      ...
      local range = module.required["treesitter"].get_node_range(first_status_extension)
...
local function task_set(character, name)
  return dorm.utils.wrap_dotrepeat(function()
