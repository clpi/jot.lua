--- @brief [[
--- Base file for mod.
--- This file contains the base implementation for "mod", building blocks of the dorm environment.
--- @brief ]]
...
local callbacks = require("dorm.util.callback")
local config = require("dorm.config")
local log = require("dorm.util.log")
local utils = require("dorm.util")

--- @alias dorm.module.public { version: string, [any]: any }

--- @class (exact) dorm.module.resolver
...
--- @class (exact) dorm.module.events
--- @field defined? { [string]: dorm.event }              Lists all events defined by this module.
--- @field subscribed? { [string]: { [string]: boolean } } Lists the events that the module is subscribed to.

--- @alias dorm.module.setup { success: boolean, requires?: string[], replaces?: string, replace_merge?: boolean, wants?: string[] }

--- Defines a module.
--- A module is an object that contains a set of hooks which are invoked by dorm whenever something in the
--- environment occurs. This can be an event, a simple act of the module being loaded or anything else.
--- @class (exact) dorm.module
--- @field config? dorm.module.configuration The configuration for the module.
--- @field events? dorm.module.events Describes all information related to events for this module.
--- @field examples? table<string, function> Contains examples of how to use the mod that users or developers may sift through.
--- @field imported? table<string, dorm.module> Imported submod of the given module. Contrary to `required`, which only exposes the public API of a module, imported mod can be accessed in their entirety.
...
--- @field dorm_post_load? fun() Function that is invoked after all mod are loaded. Useful if you want the dorm environment to be fully set up before performing some task.
...
--- @field public public? dorm.module.public Every module can expose any set of information it sees fit through this field. All functions and variables declared in this table will be visiable to any other module loaded.
--- @field required? dorm.module.resolver Contains the public tables of all mod that were required via the `requires` array provided in the `setup()` function of this module.
--- @field setup? fun(): dorm.module.setup? Function that is invoked before any other loading occurs. Should perform preliminary startup tasks.
...
function mod.load_module_from_table(module)
  ...
  if module_to_replace then
    ...
    if module_to_replace.replaced then
      log.error(
        ("Unable to replace module %s - module replacement clashing detected. This error triggers when a module tries to be replaced more than two times - dorm doesn't know which replacement to prioritize.")
        :format(
...
--- This is the recommended way of loading mod - `load_module_from_table()` should only really be used by dorm itself.
...
function mod.load_module(module_name, cfg)
  ...
  local module = require("dorm.mod." .. module_name .. ".module")
