--[[
    ...
    internal: true
    ---
This internal module exposes functionality for subscribing to autocmd and performing actions based on those autocmd.

###### NOTE: This module will be soon deprecated, and it's favourable to use the `vim.api*` functions instead.

In your `module.setup()`, make sure to require `base.autocmd` (`requires = { "autocmd" }`)
Afterwards in a function of your choice that gets called *after* base.autocommmands gets intialized (e.g. `load()`):

```lua
module.load = function()
    module.required["autocmd"].enable_autocommand("VimLeavePre") -- Substitute VimLeavePre for any valid neovim autocommand
end
```
...
local module = mod.create("autocmd")
...
---@param triggered_from_dorm boolean #If true, that means we have received this event as part of a *.dorm autocommand
---@param ev? table the original event data
function _dorm_module_autocommand_triggered(name, triggered_from_dorm, ev)
    local event = mod.create_event(module, name, { dorm = triggered_from_dorm }, ev)
    assert(event)
...
module.private.autocmd_base = function(name)
    return mod.define_event(module, name)
end
...
module.public = {
    ...
    ---@param dont_isolate boolean #base to false. Specifies whether the autocommand should run globally (* instead of in dorm files (*.dorm)
    enable_autocommand = function(autocmd, dont_isolate)
        dont_isolate = dont_isolate or false

        autocmd = autocmd:lower()
        local subscribed_autocommand = module.events.subscribed["autocmd"][autocmd]

        if subscribed_autocommand ~= nil then
            vim.cmd("augroup dorm")

            if dont_isolate and vim.fn.exists("#dorm#" .. autocmd .. "#*") == 0 then
                vim.api.nvim_create_autocmd(autocmd, {
                    callback = function(ev)
                        _dorm_module_autocommand_triggered("autocmd.events." .. autocmd, false, ev)
                    end,
                })
            elseif vim.fn.exists("#dorm#" .. autocmd .. "#*.dorm") == 0 then
                vim.api.nvim_create_autocmd(autocmd, {
                    pattern = "*.dorm",
                    callback = function(ev)
                        _dorm_module_autocommand_triggered("autocmd.events." .. autocmd, true, ev)
                    end,
                })
            end
            vim.cmd("augroup END")
            module.events.subscribed["autocmd"][autocmd] = true
        end
    end,
...
module.examples = {
    ["Binding to an Autocommand"] = function()
        local mymodule = mod.create("my.module")

        mymodule.setup = function()
            return {
                success = true,
                requires = {
                    "autocmd", -- Be sure to require the module!
                },
            }
        end
