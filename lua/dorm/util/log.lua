...
-- Modified again by Vhyrro for use with dorm :)
--
...
--- @class (exact) dorm.log.configuration
...
--- @field hl boolean                                      Whether highlighting should be used in console (using `:echohl`).
...
--- @param config dorm.log.configuration
--- @param standalone boolean
log.new = function(config, standalone)
  config = vim.tbl_deep_extend("force", base_config, config)
  config.plugin = "dorm"   -- Force the plugin name to be dorm
  ...
  local log_at_level = function(level_config, message_maker, ...)
    ...
    if config.use_console then
      ...
      if config.hl and level_config.hl then
