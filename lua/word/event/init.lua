...
local calllback = require("word.event.callback")
...
---Publish an event that is received by all modules. See the documentation for `word.events` for more information on the publishing and subscribing rules.
---@param topic string #See event_sample::topic
---@param payload any #See event_sample::payload
function C.pub(topic, payload)
  local event --[[@as word.event]] = {
    topic = topic,
    payload = payload,
...
function C.create_event(init, type, content, ev)
  ...
  local event_template =
    C.get_event_template(C.loaded_mod[mod_name] or { name = "" }, type)
...
function C.send_event(recipient, event)
  ...
  if not C.is_mod_loaded(recipient) then
    log.warn(
      "Unable to send event to init",
      recipient,
      "- the init is not loaded."
    )
    return
  end
