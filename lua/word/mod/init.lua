...
--- @alias word.module.public { version: string, [any]: any }

--- @class (exact) word.module.resolver
...
--- Defines both a public and private configuration for a word module.
...
--- @class (exact) word.module.configuration
...
--- @class (exact) word.module.events
--- @field defined? { [string]: word.event }              Lists all events defined by this module.
--- @field subscribed? { [string]: { [string]: boolean } } Lists the events that the module is subscribed to.

--- @alias word.module.setup { success: boolean, requires?: string[], replaces?: string, replace_merge?: boolean, wants?: string[] }

--- Defines a module.
--- A module is an object that contains a set of hooks which are invoked by word whenever something in the
--- environment occurs. This can be an event, a simple act of the module being loaded or anything else.
--- @class (exact) word.module
--- @field config? word.module.configuration The configuration for the module.
--- @field events? word.module.events Describes all information related to events for this module.
--- @field examples? table<string, function> Contains examples of how to use the mod that users or developers may sift through.
--- @field imported? table<string, word.module> Imported submod of the given module. Contrary to `required`, which only exposes the public API of a module, imported mod can be accessed in their entirety.
...
--- @field public public? word.module.public Every module can expose any set of information it sees fit through this field. All functions and variables declared in this table will be visiable to any other module loaded.
--- @field required? word.module.resolver Contains the public tables of all mod that were required via the `requires` array provided in the `setup()` function of this module.
--- @field setup? fun(): word.module.setup? Function that is invoked before any other loading occurs. Should perform preliminary startup tasks.
...
--- @field on_event fun(event: word.event) A callback that is invoked any time an event the module has subscribed to has fired.

local Mod = {}

--- Returns a new word module, exposing all the necessary function and variables.
...
function Mod.create(name, imports)
  ...
  local new_module = {
    ...
    events = {
      subscribed = { -- The events that the module is subscribed to
        --[[
                ["test"] = { -- The name of the module that has events bound to it
  ...
  if name then
    ...
    new_module.path = "mod." .. name
  end

  return new_module
end

--- Constructs a metamodule from a list of submod. Metamod are mod that can autoload batches of mod at once.
  ...
  module.load = function()
    module.config.public.enable = (function()
      ...
      for _, modname in ipairs(module.config.public.enable) do
        ...
        if not vim.tbl_contains(module.config.public.disable, modname) then
...
--- @param module word.module The actual module to load.
...
function Mod.load_module_from_table(module)
  ...
  if loaded_module.wants and not vim.tbl_isempty(loaded_module.wants) then
    log.info("Module", module.name, "wants certain mod. Ensuring they are loaded...")
  ...
  if module_to_replace then
    ...
    if module_to_replace.replaced then
      log.error(
        ("Unable to replace module %s - module replacement clashing detected. This error triggers when a module tries to be replaced more than two times - word doesn't know which replacement to prioritize.")
        :format(
...
--- If the module cannot not be found, attempt to load it off of github (unimplemented). This function also applies user-defined config and keys to the mod themselves.
--- This is the recommended way of loading mod - `load_module_from_table()` should only really be used by word itself.
--- @param module_name string A path to a module on disk. A path seperator in word is '.', not '/'.
--- @param cfg table? A config that reflects the structure of `word.config.user_config.load["module.name"].config`.
...
function Mod.load_module(module_name, cfg)
  ...
  local module = require("word.mod." .. module_name .. ".module")
  ...
  if not module then
    log.error(
      ...
      module_name,
      "- loaded file returned nil. Be sure to return the table created by mod.create() at the end of your module.lua file!"
    )
    return false
  end
  ...
  if module == true then
    log.error(
      ...
      module_name,
      "- loaded file didn't return anything meaningful. Be sure to return the table created by mod.create() at the end of your module.lua file!"
    )
    return false
  end
  ...
  if cfg and not vim.tbl_isempty(cfg) then
    ...
    module.config.custom = config.mod[module_name]
...
--- @param module word.module A valid table as returned by mod.create()
--- @param parent_module string|word.module If a string, then the parent is searched for in the loaded mod. If a table, then the module is treated as a valid module as returned by mod.create()
...
--- @param callback fun(module_public_table: word.module.public) The callback to execute.
function Mod.await(module_name, callback)
  ...
  callbacks.on_event("module_loaded", function(_, module)
...
--- @param module word.module A reference to the module invoking the function
...
--- Load all modules in the `lua/word/mod` directory.
