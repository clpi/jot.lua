...
M.data.data.get_links = function(source)
  ...
  if type(source) == "string" then
    ...
    if vim.fn.bufexists(source) == 1 then
      ...
      markdown_parser = vim.treesitter.get_string_parser(iter_src, "markdown")
    end
  end
  if type(source) == "number" then
    ...
    markdown_parser = vim.treesitter.get_parser(source, "markdown")
    iter_src = source
  end
  ...
  local query = vim.treesitter.query.parse("markdown", link_query_string)
  local links = {}
  for _, match in query:iter_matches(markdown_tree:root(), iter_src) do
    local link = {}
