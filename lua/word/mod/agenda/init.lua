...
M.load = function()
  M.required["core.neorgcmd"].add_commands_from_table({
    ["agenda"] = {
      args = 1,
      subcommands = {
        ["page"] = {
...
M.private = {
  ...
  insert_task_lines = function(buf_lines, header, tasks, curr_time)
    ...
    for _, task in ipairs(tasks) do
      table.insert(buf_lines, M.private.format_task_line(task, curr_time))
    end
...
M.public = {
  ...
  page_view = function(input_list)
    local task_list = M.required["external.many-mans"]["task-man"].filter_tasks(input_list)
    ...
    for _, entry in ipairs(task_list) do
      if current_file ~= entry.filename then
        ...
        local file_metadata =
            M.required["external.many-mans"]["meta-man"].extract_file_metadata(entry.filename)
        -- table.insert(buffer_lines, "___")
        table.insert(buffer_lines, "")
    ...
    local _, _ = M.required["external.many-mans"]["buff-man"].create_view_buffer(buffer_lines)
  end,
  ...
  day_view = function()
    ...
    local task_list = M.required["external.many-mans"]["task-man"].filter_tasks({
      "undone",
      "pending",
      "hold",
      "important",
    ...
    -- Sort each category
    table.sort(today, M.private.sort_today_tasks)
    table.sort(overdue, M.private.sort_by_time_and_priority)
    table.sort(scheduled, M.private.sort_by_time_and_priority)
    table.sort(next_week, M.private.sort_by_time_and_priority)
    table.sort(this_week, M.private.sort_by_time_and_priority)

    M.private.insert_task_lines(buffer_lines, "Today", today, current_time)
    M.private.insert_task_lines(buffer_lines, "Overdue", overdue, current_time)
    M.private.insert_task_lines(buffer_lines, "This Week", this_week, current_time)
    M.private.insert_task_lines(buffer_lines, "Next Week", next_week, current_time)
    M.private.insert_task_lines(buffer_lines, "Scheduled", scheduled, current_time)
  ...
  tag_view = function()
    ...
    for key, tasks in pairs(tag_task_table) do
      M.private.insert_task_lines(buffer_lines, "`" .. key .. "`", tasks, current_time)
    end
...
M.on_event = function(event)
  ...
  if event.split_type[2] == "external.agenda.page" then
    M.public.page_view(event.content)
