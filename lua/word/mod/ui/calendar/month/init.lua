...
M.data.data = {
  ...
  set_decorational_extmark = function(
    ui_info,
    row,
    col,
    length,
    ...
    return M.data.data.set_extmark(
      ui_info,
      M.data.data.namespaces.decorational,
      row,
      col,
      length,
      virt_text,
  ...
  set_logical_extmark = function(
    ...
    return M.data.data.set_extmark(
      ui_info,
      M.data.data.namespaces.logical,
      row,
      col,
      nil,
      virt_text,
  ...
  new_view_instance = function()
    return {
      ...
      render_month_banner = function(
        ...
        self.extmarks.decorational.month_headings[weekday_banner_extmark_id] = M.data.data.set_decorational_extmark(
          ui_info,
          4,
          weekday_banner_id[2]
            + math.ceil(
              (weekday_banner_id[3].end_col - weekday_banner_id[2]) / 2
            )
      ...
      render_weekday_banner = function(self, ui_info, offset, distance)
        ...
        local weekday_banner_id = M.data.data.set_decorational_extmark(
          ui_info,
          6,
          extmark_position,
      ...
      render_month = function(
        ...
        local days_in_current_month = M.data.data.get_month_length(month, year)
        ...
        for day_of_month, day_of_week in ipairs(days_of_month) do
          ...
          local start_col = beginning_of_weekday_extmark[2]
            + (4 * render_column)
      ...
      render_month_array = function(self, ui_info, date, options)
        ...
        local months_to_render =
          M.data.data.rendered_months_in_width(ui_info.width, options.distance)
        ...
        for i = 1, months_to_render do
          ...
          self:render_month_banner(
            ui_info,
            positive_target_date,
            weekday_banner
          )
      ...
      render_decorative_text = function(self, ui_info, view)
        ...
        self.extmarks.decorational =
          vim.tbl_deep_extend("force", self.extmarks.decorational, {
            calendar_text = M.data.data.set_decorational_extmark(
              ui_info,
              0,
              0,
              0,
            ...
            help_and_custom_input = M.data.data.set_decorational_extmark(
              ui_info,
              ui_info.height - 1,
              0,
              0,
            ...
            current_view = M.data.data.set_decorational_extmark(
              ui_info,
              ui_info.height - 1,
              0,
              0,
      ...
      render_view = function(self, ui_info, date, previous_date, options)
        ...
        if is_first_render then
          ...
          M.data.data.fill_buffer(ui_info)
  ...
  get_month_length = function(month, year)
    return ({
      31,
      (M.data.data.is_leap_year(year)) and 29 or 28,
      31,
      30,
      31,
      30,
...
M.data = {
  ...
  setup = function(ui_info, mode, date, options)
    ...
    local view = M.data.data.new_view_instance()
    ...
    do
      ...
      vim.keymap.set("n", "$", function()
        local new_day = date.day
          - (lib.number_wrap(date.wday - 1, 1, 7) - 1)
          + 6
        local new_date = reformat_time({
          year = date.year,
          month = date.month,
      ...
      vim.keymap.set("n", "e", function()
        local end_of_current_month =
          M.data.data.get_month_length(date.month, date.year)
        ...
        local new_date = reformat_time({
          year = date.year,
          month = date.month + vim.v.count1,
          day = M.data.data.get_month_length(
            date.month + vim.v.count1,
            date.year
          ),
        })
        view:render_view(ui_info, new_date, date, options)
      ...
      vim.keymap.set("n", "w", function()
        ...
        new_date.day = new_date.day
          - (lib.number_wrap(new_date.wday - 1, 1, 7) - 1)
        view:render_view(ui_info, new_date, date, options)
        date = new_date
      end, { buffer = ui_info.buffer })
      ...
      vim.keymap.set("n", "G", function()
        local new_date = reformat_time({
          year = date.year,
          month = date.month,
          day = M.data.data.get_month_length(date.month, date.year),
        })
        view:render_view(ui_info, new_date, date, options)
        date = new_date
      end, { buffer = ui_info.buffer })
      ...
      vim.keymap.set(
        "n",
        "?",
        lib.wrap(M.data.data.display_help, {
          {
            { "q", "@namespace" },
            { " - " },
