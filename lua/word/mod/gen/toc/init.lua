...
local inits, utils, log = word.inits, word.utils, word.log

local init = inits.create("toc")
...
init.load = function()
  inits.await("core.wordcmd", function(wordcmd)
...
init.public = {
  parse_toc_macro = function(buffer)
    ...
    local success = init.required[".treesitter"].execute_query(
      ...
      function(query, id, node)
        ...
        if
            capture_name == "name"
            and init.required[".treesitter"].get_node_text(node, buffer):lower() == "toc"
        then
          toc = true
        elseif capture_name == "parameters" and toc then
          toc_name = init.required[".treesitter"].get_node_text(node, buffer)
          return true
        end
      end,
      buffer
    ...
    local success = init.required[".treesitter"].execute_query(
      ...
      function(query, id, node)
        ...
        if prefix and title then
          local prefix_text =
              init.required[".treesitter"].get_node_text(prefix, original_buffer)
          local title_text =
              init.required[".treesitter"].get_node_text(title, original_buffer)
  ...
  update_toc = function(toc_title, ui_data, norg_buffer)
    ...
    local norg_root = init.required[".treesitter"].get_document_root(norg_buffer)
