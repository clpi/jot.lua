...
local log, inits = word.log, word.inits

local init = inits.create("todo_items")

init.setup = function()
  return { success = true, requires = { "treesitter" } }
end
...
init.load = function()
  for _, task in ipairs({
    ...
    vim.keymap.set(
      "",
      string.format("<Plug>(word.qol.todo-items.todo.task-%s)", task),
...
init.private = {
  ...
  update_parent = function(buf, line, recursion_level)
    ...
    local item_at_cursor = init.private.get_todo_item_from_cursor(buf, line)
    ...
    local range = init.required["integrations.treesitter"].get_node_range(first_status_extension)
  ...
  get_todo_item_from_cursor = function(buf, line)
    local node_at_cursor = init.required["integrations.treesitter"].get_first_node_on_line(buf, line)
  ...
  make_all = function(buf, node, todo_item_type, char)
    ...
    if init.private.get_todo_item_type(node) == todo_item_type then
      return
    end
    ...
    if not first_status_extension then
      if not init.config.public.create_todo_items then
        return
      end
      ...
      local range = init.required["treesitter"].get_node_range(first_status_extension)
    ...
    for child in node:iter_children() do ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
      if type == child:type():match("^(.+)%d+$") then
        init.private.make_all(buf, child, todo_item_type, char)
      end
    end
  end,

  task_cycle = function(buf, linenr, types, alternative_types)
    local todo_item_at_cursor = init.private.get_todo_item_from_cursor(buf, linenr - 1)

    if not todo_item_at_cursor then
      return
    end

    local todo_item_type = init.private.get_todo_item_type(todo_item_at_cursor)
    ...
    if not todo_item_type then
      ...
      init.private.make_all(buf, todo_item_at_cursor, types[1][1], types[1][2])
    ...
    for child in todo_item_at_cursor:iter_children() do
      if init.private.get_todo_item_type(child) then
        next = alternative_types[get_index(alternative_types, todo_item_type)] or alternative_types[1]
        break
      end
    end

    init.private.make_all(buf, todo_item_at_cursor, next[1], next[2])
    init.private.update_parent(buf, linenr - 1, 0)
  end,
}
...
local function task_set(character, name)
  return word.utils.wrap_dotrepeat(function()
    ...
    local todo_item_at_cursor = init.private.get_todo_item_from_cursor(buffer, cursor[1] - 1)

    if not todo_item_at_cursor then
      return
    end

    init.private.make_all(buffer, todo_item_at_cursor, name, character)
  end)
end
