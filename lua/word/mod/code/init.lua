--[[
    ...
    description: The `base.code` init exports code blocks within a `.word` file straight to a file of your choice.
    summary: An Advanced Code Block Exporter.
    ---
The goal of this init is to allow users to spit out the contents of code blocks into
many different files. This is the primary component required for a literate configuration in word,
where the configuration is annotated and described in a `.word` document, and the actual code itself
is thrown out into a file that can then be normally consumed by e.g. an application.

The `code` init currently provides a single command:
- `:word code current-file` - performs all possible tangling operations on the current file
...
The above snippet tells the word tangling engine to code all `lua` code blocks to `./init.lua` and all `haskell` code blocks to `./output.hs`.
...
* `file-content` -- Try to determine the filetype of the codeblock and insert the word file content as a delimiter.
...
init.public = {
    code = function(buffer)
        ...
        local treesitter = init.required["treesitter"]
        ...
        local query = utils.ts_parse_query("word", query_str)
