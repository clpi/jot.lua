...
M.data = {
  ...
  goto_next_query_match = function(query_string)
    ...
    local document_root = M.data.get_document_root(0)
    ...
    for id, node in next_match_query:iter_captures(document_root, 0, line_number - 1, -1) do
      if next_match_query.captures[id] == "next-segment" then
        ...
        if (start_line == line_number and start_col > col_number) or start_line > line_number then
          M.data.data.os_utils.goto_node(node) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
          return
        end
      end
  ...
  goto_previous_query_match = function(query_string)
    ...
    if final_node then
      M.data.data.os_utils.goto_node(final_node) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
    end
  end,
  ...
  get_all_nodes_in_file = function(node_type, path, filetype)
    ...
    return M.data.search_tree(tree, node_type)
  end,
  ...
  swap_nodes = function(node1, node2, bufnr, cursor_to_second)
    ...
    local range1 = M.data.node_to_lsp_range(node1)
    local range2 = M.data.node_to_lsp_range(node2)

    local text1 = M.data.get_node_text(node1, bufnr)
    local text2 = M.data.get_node_text(node2, bufnr)
      ...
      if vim.endswith(child:type(), "_carryover_set") then
        for subchild in child:iter_children() do
          if vim.endswith(subchild:type(), "_carryover") then
            local meta = M.data.get_tag_info(subchild, throw)
  ...
  get_first_node_on_line = function(buf, line, stop_type)
    ...
    local document_root = M.data.get_document_root(buf)
  ...
  get_document_metadata = function(source, no_trim)
    ...
    local function parse_data(node, src)
      return lib.match(node:type())({
        string = function()
          return trim(M.data.get_node_text(node, src))
        end,
        number = function()
          return tonumber(M.data.get_node_text(node, src))
        end,
        array = function()
        ...
        object = function()
          ...
          for child in node:iter_children() do
            ...
            local key_content = trim(M.data.get_node_text(key, src))
    ...
    for id, node in meta_query:iter_captures(markdown_inline_tree:root(), meta_source) do
      if meta_query.captures[id] == "key" then
        local key = trim(M.data.get_node_text(node, meta_source))

        local val
        if key == "title" then
          -- force title's value as string type
          val = trim(M.data.get_node_text(node:next_named_sibling(), meta_source))
        else
