--[[
    ...
    summary: Easily track a template within word.
    ---
The template M exposes a total of six commands.
The first three, `:word template today|yesterday|tomorrow`, allow you to access entries
for a given time relative to today. A file will be opened with the respective date as a `.word` file.

The fourth command, `:word template custom`, allows you to specify a custom date as an argument.
The date must be formatted according to the `YYYY-mm-dd` format, e.g. `2023-01-01`.

The `:word template template` command creates a template file which will be used as the base whenever
a new template entry is created.

Last but not least, the `:word template toc open|update` commands open or create/update a Table of Contents
...
M.private = {
  ...
  open_template = function(time, custom_date)
    ...
    local folder_name = M.config.public.template_folder
  ...
  template_tomorrow = function()
    M.private.open_template(os.time() + 24 * 60 * 60)
  end,
                ...
                if dtype == "file" and string.match(dname, "%d%d%.md") then
                  ...
                  vim.schedule(function()
                    ...
                    table.insert(toc_entries, {
                      ...
                      .. config.pathsep
                      .. M.config.public.template_folder
                      .. config.pathsep
                      .. name
                      .. config.pathsep
                      .. mname
...
M.on_event = function(event)
  if event.split_type[1] == "cmd" then
    ...
    elseif event.split_type[2] == "template.tomorrow" then
      M.private.template_tomorrow()
    elseif event.split_type[2] == "template.yesterday" then
      M.private.template_yesterday()
    elseif event.split_type[2] == "template.custom" then
      if not event.content[1] then
        ...
        M.private.open_template(nil, event.content[1])
      end
    elseif event.split_type[2] == "template.today" then
      M.private.template_today()
...
M.examples = {
  ["Changing TOC format to divide year in quarters"] = function()
    -- In your ["template"] options, change toc_format to a function like this:
