...
module.load = function()
  ...
  vim.api.nvim_create_user_command("Word", module.private.command_callback, {
...
module.public = {
  ...
  --- Recursively merges the contents of the module's config.public.funtions table with base.cmd's module.config.public.word_commands table.
  ---@param module_name string #An absolute path to a loaded module with a module.config.public.word_commands table following a valid structure
  add_commands = function(module_name)
    ...
    module.public.word_commands =
        vim.tbl_extend("force", module.public.word_commands, module_config.word_commands)
  end,

  --- Recursively merges the provided table with the module.config.public.word_commands table.
  ---@param functions table #A table that follows the module.config.public.word_commands structure
  add_commands_from_table = function(functions)
    module.public.word_commands = vim.tbl_extend("force", module.public.word_commands, functions)
  end,
  ...
  add_commands_from_file = function(name)
    ...
    local err, ret = pcall(require, "word.mod.cmd.commands." .. name .. ".module")
  ...
  sync = function()
    ...
    for _, mod in pairs(mod.loaded_mod) do
      if mod.public.word_commands then
        module.public.add_commands_from_table(mod.public.word_commands)
      end
    end
  end,
...
module.private = {
  ...
  command_callback = function(data)
    ...
    local ref = {
      subcommands = module.public.word_commands,
    }
    ...
    if #args == 0 or argument_count < ref.min_args then
      local completions = module.private.generate_completions(_, table.concat({ "word ", data.args, " " }))
    ...
    if not ref.name then
      log.error(
        ("Error when executing `:word %s` - the ending command didn't have a `name` variable associated with it! This is an implementation error on the developer's side, so file a report to the author of the module.")
        :format(
          data.args
        )
      )
...
module.word_post_load = module.public.sync
