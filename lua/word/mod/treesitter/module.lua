--[[
    ...
    summary: A module designed to integrate Treesitter into word.
...
local lib, log, modules, utils = word.lib, word.log, word.mod, word.utils
...
module.load = function()
  ...
  if module.config.public.configure_parsers then
    ...
    -- parser_configs.word = {
    -- install_info = module.config.public.parser_configs.word,
    -- }

    -- parser_configs.word_meta = {
    -- install_info = module.config.public.parser_configs.word_meta,
    -- }

    modules.await("cmd", function(wordcmd)
    ...
    vim.api.nvim_create_autocmd("BufEnter", {
      ...
      once = true,
      callback = function()
        module.public.parser_path = vim.api.nvim_get_runtime_file("parser/word.so", false)[1]
...
module.public = {
  ...
  get_document_metadata = function(source, no_trim)
    ...
    local word_parser, iter_src = module.public.get_ts_parser(source)
