...
function _word_module_autocommand_triggered(name, triggered_from_word, ev)
    local event = mod.create_event(module, name, { word = triggered_from_word }, ev)
    assert(event)
        ...
        if subscribed_autocommand ~= nil then
            ...
            if dont_isolate and vim.fn.exists("#word#" .. autocmd .. "#*") == 0 then
                vim.api.nvim_create_autocmd(autocmd, {
                    callback = function(ev)
                        _word_module_autocommand_triggered("autocmd.events." .. autocmd, false, ev)
                    end,
                })
            elseif vim.fn.exists("#word#" .. autocmd .. "#*.word") == 0 then
                vim.api.nvim_create_autocmd(autocmd, {
                    pattern = "*.word",
                    callback = function(ev)
                        _word_module_autocommand_triggered("autocmd.events." .. autocmd, true, ev)
                    end,
                })
            end
            vim.cmd("augroup END")
