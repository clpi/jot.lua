...
--- @param name string The name of the new init. Make sure this is unique. The recommended naming convention is `category.mod_name` or `category.subcategory.mod_name`.
...
--- @return word.mod
function M.create(name, imports)
  ---@type word.mod
  ...
  if imports then
    for _, import in ipairs(imports) do
      ...
      new_mod.imported[fullpath] = M.loaded_mod[fullpath]
    end
  end
...
--- @param name string The name of the new metainit. Make sure this is unique. The recommended naming convention is `category.mod_name` or `category.subcategory.mod_name`.
--- @param ... string A list of init names to load.
--- @return word.mod
M.create_meta = function(name, ...)
  local m = M.create(name)
...
function M.setup_mod_from_table(m)
  ...
  if mod_load.replaces and mod_load.replaces ~= "" then
    mod_to_replace = vim.deepcopy(M.loaded_mod[mod_load.replaces])
  end
  ...
  if mod_load.wants and not vim.tbl_isempty(mod_load.wants) then
    ...
    for _, req_mod in ipairs(mod_load.wants) do
      ...
      m.required[req_mod] = M.loaded_mod[req_mod].public
    end
  end
  ...
  M.loaded_mod_count = M.loaded_mod_count + 1
...
--- @return T?
function M.get_mod(mod_name)
  if not M.is_mod_loaded(mod_name) then
    log.trace("Attempt to get init with name", mod_name, "failed - init is not loaded.")
    return
  end

  return M.loaded_mod[mod_name].public
end
...
function M.get_mod_version(mod_name)
  ...
  local version = M.get_mod(mod_name).version
...
function M.await(mod_name, callback)
  if M.is_mod_loaded(mod_name) then
    callback(assert(M.get_mod(mod_name)))
    return
  end
...
--- @field mode Me The mode Neovim was in at the moment of broadcasting the event.
...
--- @return string[]?
function M.split_event_type(type)
...
function M.get_event_template(init, type)
  ...
  return M.loaded_mod[init.name].events.defined[split_type[2]]
end
...
--- @return word.event
function M.define_event(init, name)
...
function M.create_event(init, type, content, ev)
  -- Get the init that contains the event
  local mod_name = M.split_event_type(type)[1]

  -- Retrieve the template from init.events.defined
  local event_template = M.get_event_template(M.loaded_mod[mod_name] or { name = "" }, type)
  ...
  new_event.split_type = assert(M.split_event_type(type))
...
function M.broadcast_event(event, callback)
  ...
  for _, current_init in pairs(M.loaded_mod) do
...
function M.send_event(recipient, event)
  ...
  local modl = M.loaded_mod[recipient]
