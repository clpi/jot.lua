--- @class jot.C
local C = {
  ---@type table<string, { [1]: fun(event: jot.event, content: table|any), [2]?: fun(event: jot.event): boolean }>
  callback_list = {},
}
...
--- @param callback fun(event: jot.event, content: table|any) The function to call whenever our event gets triggered.
--- @param content_filter? fun(event: jot.event): boolean # A filtering function to test if a certain event meets our expectations.
function C.on_event(event_name, callback, content_filter)
  -- If the table doesn't exist then create it
  C.callback_list[event_name] = C.callback_list[event_name] or {} -- Insert the callback and content filter
  require("table").insert(C.callback_list[event_name], { callback, content_filter })
end

--- Used internally by jot to call all C with an event.
--- @param event jot.event An event as returned by `mod.create_event()`
