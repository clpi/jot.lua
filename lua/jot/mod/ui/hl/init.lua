...
jot under a single tree of highlight: `@jot.*`.
--]]
...
init.config.public = {
  ...
  -- trees is the final highighlightight to apply to that tree. For example: `"+@comment"` tells jot to
  ...
  -- ```lua
  -- @jot.tags.ranged_verbatim.begin
  -- ```
  -- and converts into the following command:
  -- ```vim
  -- highighlightight! link @jot.tags.ranged_verbatim.begin @comment
  -- ```
  highlight = {
    -- highlight displayed in jot selection window popups.
    selection_window = {
      heading = "+@annotation",
      arrow = "+@none",
      key = "+@init",
    ...
    --
    -- This strictly covers the `( )` component of any detached modifier. In other jots, these
    ...
    link = {
      ...
      location = {
        ...
        footnote = {
          [""] = "+@jot.footnotes.title",
          prefix = "+@jot.footnotes.prefix",
        },

        heading = {
          ["1"] = {
            [""] = "+@jot.headings.1.title",
            prefix = "+@jot.headings.1.prefix",
          },

          ["2"] = {
            [""] = "+@jot.headings.2.title",
            prefix = "+@jot.headings.2.prefix",
          },

          ["3"] = {
            [""] = "+@jot.headings.3.title",
            prefix = "+@jot.headings.3.prefix",
          },

          ["4"] = {
            [""] = "+@jot.headings.4.title",
            prefix = "+@jot.headings.4.prefix",
          },

          ["5"] = {
            [""] = "+@jot.headings.5.title",
            prefix = "+@jot.headings.5.prefix",
          },

          ["6"] = {
            [""] = "+@jot.headings.6.title",
            prefix = "+@jot.headings.6.prefix",
          },
        },
      },
    },
...
init.public = {
  ...
  trigger_highlight = function()
    ...
    --     if not query.has_highlight("jot") then
    --         query.invalidate_query_cache()

    --         if not query.has_highlight("jot") then
    --             log.error(
    --                 "nvim-treesitter has no available highlight for jot! Ensure treesitter is properly loaded in your config."
    --             )
    --         end
    --     end

    --     if vim.bo.filetype == "jot" then
    --         require("nvim-treesitter.highighlightight").attach(vim.api.nvim_get_current_buf(), "jot")
    --     end
    -- end
