--[[
    ...
    summary: Parses and handles dates in jot.
    internal: true
    ---
...
- `jot.time.insert-date` - Insert date at cursor position (called from normal mode)
- `jot.time.insert-date.insert-mode` - Insert date at cursor position (called from insert mode)
--]]
...
init.public = {
  ...
  --- Converts a lua `osdate` to a jot date.
  ...
  ---@param input string #The input which should follow the date specification found in the jot spec.
  ...
  parse_date = function(input)
    ...
    for jot in vim.gsplit(input, "%s+") do
      if jot:len() == 0 then
        goto continue
      end

      if jot:match("^-?%d%d%d%d+$") then
        output.year = tonumber(jot)
      elseif jot:match("^%d+%w*$") then
        output.day = tonumber(jot:match("%d+"))
      elseif vim.list_contains(timezone_list, jot:upper()) then
        output.timezone = jot:upper()
      else
        do
          local hour, minute, second = vim.re.match(jot, time_regex)
        ...
        do
          ...
          for i, month in ipairs(months) do
            if vim.startswith(month, jot:lower()) then
        ...
        do
          ...
          for i, weekday in ipairs(weekdays) do
            if vim.startswith(weekday, jot:lower()) then
...
init.load = function()
  vim.keymap.set("", "<Plug>(jot.time.insert-date)", lib.wrap(init.public.insert_date, false))
  vim.keymap.set("i", "<Plug>(jot.time.insert-date.insert-mode)", lib.wrap(init.public.insert_date, true))
end
