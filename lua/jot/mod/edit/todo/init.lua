...
local log, inits = jot.log, jot.mod
...
init.load = function()
  for _, task in ipairs({
    ...
    vim.keymap.set(
      "",
      string.format("<Plug>(jot.insert.todo.todo.task-%s)", task),
...
local function task_set(character, name)
  return jot.utils.wrap_dotrepeat(function()
    local buffer = vim.api.nvim_get_current_buf()
    local cursor = vim.api.nvim_win_get_cursor(0)

    local todo_at_cursor =
        init.public.data.get_todo_from_cursor(buffer, cursor[1] - 1)

    if not todo_at_cursor then
      return
    end

    init.public.data.make_all(buffer, todo_at_cursor, name, character)
  end)
end
...
init.public = {
  data = {
    ...
    update_parent = function(buf, line, recursion_level)
      ...
      local item_at_cursor = init.public.data.get_todo_from_cursor(buf, line)
      ...
      local first_status_extension = init.public.data.find_first_status_extension(
        item_at_cursor:named_child(1)
      )

      -- TODO(vhyrro):
      -- Implement a toggleable behaviour where jot can automatically convert this:
      --     * (@ Mon 5th Feb) Test
      --     ** ( ) Test
      -- To this:
      --     * (x|@ Mon 5th Feb) Test
      ...
      if not first_status_extension then
        if not init.config.create_todo_parents then
          return
        end
        ...
        init.public.data.update_parent(buf, line, recursion_level + 1)
        return
      end

      local range = init.required["integration.treesitter"].get_node_range(
    ...
    get_todo_from_cursor = function(buf, line)
      local node_at_cursor =
          init.required["integration.treesitter"].get_first_node_on_line(buf, line)
    ...
    get_todo_type = function(todo_node)
      ...
      local todo_type =
          init.public.data.find_first_status_extension(todo_node:named_child(1)) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
    ...
    make_all = function(buf, node, todo_type, char)
      ...
      if init.public.data.get_todo_type(node) == todo_type then
        return
      end

      local first_status_extension =
          init.public.data.find_first_status_extension(node:named_child(1)) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>

      if not first_status_extension then
        if not init.config.create_todos then
          return
        end
      ...
      for child in node:iter_children() do ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
        if type == child:type():match("^(.+)%d+$") then
          init.public.data.make_all(buf, child, todo_type, char)
        end
      end
    end,
    ...
    task_cycle = function(buf, linenr, types, alternative_types)
      local todo_at_cursor =
          init.public.data.get_todo_from_cursor(buf, linenr - 1)

      if not todo_at_cursor then
        return
      end

      local todo_type = init.public.data.get_todo_type(todo_at_cursor)
      ...
      if not todo_type then
        ...
        init.public.data.make_all(buf, todo_at_cursor, types[1][1], types[1][2])
        init.public.data.update_parent(buf, linenr - 1, 0)
        return
      end
      ...
      for child in todo_at_cursor:iter_children() do
        if init.public.data.get_todo_type(child) then
      ...
      init.public.data.make_all(buf, todo_at_cursor, next[1], next[2])
      init.public.data.update_parent(buf, linenr - 1, 0)
    end,
  },
  ...
  ["task-cycle"] = function()
    ...
    init.public.data.task_cycle(
      buffer,
      cursor[1],
      init.config.order,
      init.config.order_with_children
    )
  end,
  ["task-cycle-reverse"] = function()
    ...
    init.public.data.task_cycle(
      buffer,
      cursor[1],
      vim.fn.reverse(init.config.order),
      vim.fn.reverse(init.config.order_with_children)
    )
  end,
}
