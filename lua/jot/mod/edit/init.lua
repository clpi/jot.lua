local M = require("jot.mod").create("edit", {
  "toc", "todo",
  "indent",
  "link",
})
...
M.public.find_patterns = function(str, patterns, reverse, init)
...
--]]
M.public.jump = function(pattern, reverse)
  ...
  while continue do
    ...
    if left and right then
      ...
      then
        ...
        left, right = M.public.find_patterns(line, pattern, reverse, reverse and left or right)
      end
...
--]]
M.public.go_to_heading = function(anchor_text, reverse)
...
M.public.go_to_id = function(id, starting_row)
...
pattern.
--]]
M.public.toNextLink = function(pattern)
  M.jump(M.public.jump_patterns)
end
...
link pattern.
--]]
M.public.toPrevLink = function(pattern)
  M.jump(M.public.jump_patterns, true)
end

--[[
toHeading() finds a particular heading in the file
--]]
M.public.toHeading = function(anchor_text, reverse)
  M.public.go_to_heading(anchor_text, reverse)
end

M.public.toId = function(id, starting_row)
  return M.public.go_to_id(id, starting_row)
end
...
--]]
M.public.yankAsAnchorLink = function(full_path)
  ...
  -- See if the line starts with a hash
  local is_heading = string.find(line[1], '^#')
  local is_bracketed_span = M.required.links.getBracketedSpanPart()
  if is_heading then
    -- Format the line as an anchor link
    local anchor_link = M.required.links.formatLink(line[1])
