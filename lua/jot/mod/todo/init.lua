...
init.load = function()
  vim.api.nvim_create_autocmd("Filetype", {
    pattern = "markdown",
    desc = "Attaches the TODO introspector to any jot buffer.",
...
--- Attaches the introspector to a given jot buffer.
--- Errors if the target buffer is not a jot buffer.
...
function init.public.attach_introspector(buffer)
  ...
  then
    error(
      string.format(
        "Could not attach to buffer %d, buffer is not a jot file!",
        buffer
      )
    )
  end
