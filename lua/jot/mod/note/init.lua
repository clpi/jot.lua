--[[
    ...
    summary: Easily track a note within jot.
    ---
The note M exposes a total of six commands.
The first three, `:jot note today|yesterday|tomorrow`, allow you to access entries
for a given time relative to today. A file will be opened with the respective date as a `.jot` file.

The fourth command, `:jot note custom`, allows you to specify a custom date as an argument.
The date must be formatted according to the `YYYY-mm-dd` format, e.g. `2023-01-01`.

The `:jot note template` command creates a template file which will be used as the base whenever
a new note entry is created.

Last but not least, the `:jot note toc open|update` commands open or create/update a Table of Contents
...
local config, lib, log, mod =
    require("jot.config").config, jot.lib, jot.log, jot.mod
...
M.public = {
  ...
  capture = function()
    local b, w = M.required["ui.win"].win("today", "note", "Jot note today")
    -- vim.cmd

    -- Mod.get_mod("ui.win").cmd(w, function()
    --   vim.api.nvim_command("Jot note today")
    -- end)
    -- pcall(vim.api.nvim_command, ":Jot note today")
  end,
  --- Opens a note entry at the given time
  ---@param time? number #The time to open the note entry at as returned by `os.time()`
  ---@param custom_date? string #A YYYY-mm-dd string that specifies a date to open the note at instead
  ...
  note_tomorrow = function()
    M.public.open_note(os.time() + 24 * 60 * 60)
  end,
  ...
  year_prev = function()
    M.public.open_note(os.time() - 24 * 60 * 60 * 365)
  end,
  year_next = function()
    M.public.open_note(os.time() + 24 * 60 * 60 * 365)
  end,
  month_prev = function()
    M.public.open_note(os.time() - 24 * 60 * 60 * 30)
  end,
  ...
  week_prev = function()
    M.public.open_note(os.time() - 24 * 60 * 60 * 7)
  end,
...
M.config = {
  ...
  -- The name of the template file to use when running `:jot note template`.
  template = {
...
function M.public:number_to_weekday(n)
  if n ~= nil then
    return M.public.weekday[n]
  end
end

M.public.number_to_month = function(n)
  return M.public.months[n]
end
...
M.public.weekday = tonumber(os.date("%w", os.time(M.public.timetable)))
M.setup = function()
  return {
    loaded = true,
    requires = {
