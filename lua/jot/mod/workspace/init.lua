...
local log, mod, utils = jot.log, require("jot.mod"), jot.utils
...
M.config.public = {
  ...
  -- without arguments.
  --
  -- May also be set to the string `"default"`, due to which jot will always
...
M.public = {
  ...
  ---@param path string|PathlibPath # path
  ---@param raw_path boolean? # If true, returns resolved path, otherwise, returns resolved path and append ".jot"
  ...
  --- Updates completions for the :jot command
  sync = function()
  ...
  --- Takes in a path (can include directories) and creates a .jot file from that path
  ---@param path string|PathlibPath a path to place the .jot file in
  ...
  ---@param path string|PathlibPath #A path to open the file (e.g directory/filename.jot)
  ...
  ---@param workspace_name string
  ---@return PathlibPath[]|nil
  get_jot_files = function(workspace_name)
    local res = {}
    local workspace = M.public.get_workspace(workspace_name)
...
M.on_event = function(event)
  -- If somebody has executed the :jot workspace command then
  ...
  -- If somebody has executed the :jot index command then
