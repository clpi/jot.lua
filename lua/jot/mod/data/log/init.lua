...
M.setup = function()
  vim.api.nvim_create_namespace("jot/data/log")
  return {
    loaded = true,
    requires = {
      "ui.popup",
...
M.public.datam = M.required["data"]
...
M.config = {
  ...
  -- The name of the template file to use when running `:jot log template`.
...
M.on_event = function(event)
  if event.split_type[1] == "cmd" then
    ...
    elseif event.split_type[2] == "log.tomorrow" then
      M.public.data.log_tomorrow()
    elseif event.split_type[2] == "log.yesterday" then
      M.public.data.log_yesterday()
    elseif event.split_type[2] == "log.custom" then
      if not event.content[1] then
        ...
        M.public.data.open_log(nil, event.content[1])
      end
    elseif event.split_type[2] == "log.new" then
      M.public.data.log_new()
