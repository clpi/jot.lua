...
M.load = function()
  ...
  if M.config.configure_parsers then
    ...
    --   once = true,
    --   callback = function()
    --     M.public.parser_path = vim.api.nvim_get_runtime_file("parser/jot.so", false)[1]
    --
    ...
    --       assert(
    --         false,
    --         "jot's parser is not installed! Run `:jot sync-parsers` to install it, then restart Neovim."
    --       )
    --     end
    --   end,
    -- })
...
M.config = {
  ...
  --- If true will automatically install jot parsers if they are not present.
  ...
  -- parser_configs = {
  -- Configuration for the mainline jot parser.
  -- jot = {
  --   url = "https://github.com/nvim-jot/tree-sitter-jot",
  ...
  -- markdown_inline = {
  --   url = "https://github.com/nvim-jot/tree-sitter-jot-meta",
  --   files = { "src/parser.c" },
  --   branch = "main",
...
M.public = {
  ...
  ---@param filetype string? file type of the file or `jot` if omitted
  ...
  get_document_metadata = function(source, no_trim)
    ...
    local jot_parser, iter_src = M.public.get_ts_parser(source)
    if not jot_parser then
      return
    end

    local jot_tree = jot_parser:parse()[1]
    if not jot_tree then
      return
    end
    ...
    local jot_query = utils.ts_parse_query(
      "markdown",
      [[
                (document
                  (ranged_verbatim_tag
    ...
    local meta_node
    for id, node in jot_query:iter_captures(jot_tree:root(), iter_src) do
      if jot_query.captures[id] == "tag_name" then
  ...
  --- Parses a query and automatically executes it for jot
  ...
  execute_query = function(query_string, callback, source, start, finish)
    ...
    local root = jot_parser:parse()[1]:root()
  ...
  ---Create a jot TS parser from the given source
  ---@param source string | number | PathlibPath file path or buf number or 0 for current buffer
  ---@return vim.treesitter.LanguageTree? jot_parser
  ...
  get_ts_parser = function(source)
    ...
    if type(source) == "string" then
      ...
      if vim.fn.bufnr(source) ~= -1 then ---@diagnostic disable-line
        ...
        jot_parser = vim.treesitter.get_string_parser(iter_src, "markdown")
      end
    end
    if type(source) == "number" then
      ...
      jot_parser = vim.treesitter.get_parser(source, "markdown")
      iter_src = source
    end

    return jot_parser, iter_src
  end,
}

-- this fixes the problem of installing jot ts parsers on macOS without resorting to using gcc
local function install_jot_ts()
  local install = require("nvim-treesitter.install")

  if vim.fn.has("macunix") == 1 then
    -- https://github.com/nvim-jot/tree-sitter-jot/issues/7
...
M.on_event = function(event)
  if event.split_type[2] == "sync" then
    local ok, err = pcall(install_jot_ts)

    if not ok then
      utils.notify(string.format([[Unable to auto-install jot parser: %s]], err), vim.log.levels.WARN)
    end
