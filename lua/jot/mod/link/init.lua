  ...
  createLink = function(args)
    ...
    if mode == "n" and not range then
      ...
      if url_start and url_end then
        ...
        -- Find the (first) position of the matched jot in the line
        local left, right = string.find(line, cursor_jot, nil, true)
...
link from the jot under the cursor or a visual selection (if there is one).
--]]
