...
--- Defines both a public and private configuration for a jot init.
--- Public configurations may be tweaked by the user from the `jot.setup()` function,  whereas private configurations are for internal use only.
--- @class (exact) jot.mod.configuration
...
--- @class (exact) jot.mod
--- @field config? jot.mod.configuration The configuration for the init.
...
_G.Mod.default_mod = function(name)
  return {
    ...
    private = {},
    public = {
      version = require("jot.config").config.version,
    },
    config = {
      private = {},
      public = {},
...
function Mod.load_mod_from_table(m)
  ...
  if mod_to_replace then
    ...
    if mod_to_replace.replaced then
      log.error(
        ("Unable to replace init %s - init replacement clashing detected. This error triggers when a init tries to be replaced more than two times - jot doesn't know which replacement to prioritize.")
        :format(
          mod_to_replace.name
        )
      )
...
--- @param mod_name string A path to a init on disk. A path seperator in jot is '.', not '/'.
...
function _G.Mod.load_mod(mod_name, cfg)
  ...
  local modl = require("jot.mod." .. mod_name)
