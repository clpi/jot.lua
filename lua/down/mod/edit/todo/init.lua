...
M.load = function()
  for _, task in ipairs({
    ...
    vim.keymap.set(
      "",
      string.format("<Plug>(down.insert.todo.todo.task-%s)", task),
      M.data["task-" .. task]
    )
  end
end
...
local function task_set(character, name)
  return down.utils.wrap_dotrepeat(function()
    ...
    M.data.data.make_all(buffer, todo_at_cursor, name, character)
  end)
end
...
M.data = {
  data = {
    ...
    update_parent = function(buf, line, recursion_level)
      ...
      local item_at_cursor = M.data.data.get_todo_from_cursor(buf, line)
    ...
    make_all = function(buf, node, todo_type, char)
      ...
      if M.data.data.get_todo_type(node) == todo_type then
        return
      end
      ...
      for child in node:iter_children() do ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
        if type == child:type():match("^(.+)%d+$") then
          M.data.data.make_all(buf, child, todo_type, char)
        end
      end
    end,

    task_cycle = function(buf, linenr, types, alternative_types)
      local todo_at_cursor = M.data.data.get_todo_from_cursor(buf, linenr - 1)

      if not todo_at_cursor then
        return
      end

      local todo_type = M.data.data.get_todo_type(todo_at_cursor)
      ...
      if not todo_type then
        ...
        M.data.data.make_all(buf, todo_at_cursor, types[1][1], types[1][2])
      ...
      M.data.data.make_all(buf, todo_at_cursor, next[1], next[2])
      M.data.data.update_parent(buf, linenr - 1, 0)
    end,
  },
