...
M.data = {
    ...
    for link_type, pattern in pairs(patterns) do
      ...
      while continue do
        ...
        if start_row then -- There's a match
          local overlaps = M.data.contains(
            start_row,
            start_col,
            end_row,
            end_col,
  ...
  createLink = function(args)
    ...
    if mode == "n" and not range then
      ...
      local url_start, url_end = M.data.hasUrl(line, "positions", col)
      if url_start and url_end then
        ...
        local cursor_down = vim.fn.expand("<cdown>")
  ...
  destroyLink = function()
    ...
    local link = M.data.getLinkUnderCursor()
    if link then
      local link_name = M.data.getLinkPart(link, "name")
