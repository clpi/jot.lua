...
---@class lsp.document.format
M.data = {
  ---@type lsp.FormattingOptions
  opts = {
  ...
  document = {
    ...
    ---@type lsp.DocumentFormattingRegistrationOptions
    registration = {
      documentSelector = {
        scheme = "file",
        language = "markdown",
    ...
    ---@type lsp.DocumentFormattingOptions
    opts = {
...
M.format_document = function() end
...
M.data = {
  ...
  rename_file = function(current_path, new_path)
    ...
    ---@param link Link
    local current_file_changes = M.data.data.fix_links(
      current_path,
      function(link)
    ...
    local files = workspace.get_md_files(ws_name)
    ...
    for _, file in ipairs(files) do
      ...
      ---@param link Link
      local file_changes = M.data.data.fix_links(file, function(link)
        local range = link.file and link.file.range
        local link_str = link.file and link.file.text
        local raw = false
        if link.type and link.type.text == "/ " then
  ...
  rename_heading = function(line_number, new_heading)
    ...
    local wsEdit = { changes = {} }
    ---@param link Link
    local changes = M.data.data.fix_links(buf, function(link)
    ...
    for _, file in ipairs(files) do
      ...
      ---@param link Link
      changes = M.data.data.fix_links(file, function(link)
        local link_str = link.file and link.file.text
...
M.data.data.fix_links = function(source, fix_link)
  local links = nil
  links = M.data.data.get_links(source)
...
---@return Link[]
M.data.data.get_links = function(source)
  ...
  if type(source) == "string" then
    ...
    if vim.fn.bufexists(source) == 1 then
      ...
      md_parser = vim.treesitter.get_string_parser(iter_src, "md")
    end
  end
  if type(source) == "number" then
    ...
    md_parser = vim.treesitter.get_parser(source, "md")
    iter_src = source
  end
  ...
  local query = vim.treesitter.query.parse("md", link_query_string)
  local links = {}
  for _, match in query:iter_matches(md_tree:root(), iter_src) do
    local link = {}
