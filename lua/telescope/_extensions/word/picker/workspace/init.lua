...
local word_loaded, word = pcall(require, "word")

assert(word_loaded, "word is not loaded - please make sure to load word first")

local ns = vim.api.nvim_create_namespace("word-tele-workspace-preview")
...
return function(options)
  if not workspaces then
    ...
    local workspaces_raw = word.mod.get_mod("workspace").get_workspaces()
      ...
      .new(opts, {
        ...
        attach_mappings = function(prompt_bufnr)
          action_set.select:replace(function()
            ...
            if entry then
              word.mod.get_mod("workspace").open_workspace(entry.value.name)
            end
          end)
          return true
        end,
