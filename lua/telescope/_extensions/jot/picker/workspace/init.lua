...
local jot_loaded, jot = pcall(require, "jot")

assert(jot_loaded, "jot is not loaded - please make sure to load jot first")

local ns = vim.api.nvim_create_namespace("jot-tele-workspace-preview")
...
return function(options)
  if not workspaces then
    ...
    local workspaces_raw = jot.mod.get_mod("workspace").get_workspaces()
      ...
      .new(opts, {
        ...
        previewer = previewers.new_buffer_previewer({
          define_preview = function(self, entry, status)
            ...
            if not files[workspace.name] then
              files[workspace.name] = jot.mod.get_mod("workspace").get_jot_files(workspace.name)
            end
        ...
        attach_mappings = function(prompt_bufnr)
          action_set.select:replace(function()
            ...
            if entry then
              jot.mod.get_mod("workspace").open_workspace(entry.value.name)
            end
          end)
          return true
        end,
